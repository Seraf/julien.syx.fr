export default {
  // Nav
  'nav.home': 'Accueil',
  'nav.parcours': 'Parcours',
  'nav.experience': 'Expérience',
  'nav.realisations': 'Réalisations',
  'nav.engagements': 'Engagements',
  'nav.contact': 'Contact',

  // Hero
  'hero.greeting': 'Bonjour, je suis',
  'hero.name': 'Julien Syx',
  'hero.title': 'CTO / Product Lead',
  'hero.subtitle': 'Cycloid',
  'hero.description': 'Passionné d\'informatique depuis l\'âge de 8 ans, premier salarié et associé de Cycloid depuis 2015. Je porte la stratégie produit et dirige une équipe de 15 personnes en full remote à travers l\'Europe.',
  'hero.cta.parcours': 'Découvrir mon parcours',
  'hero.cta.contact': 'Me contacter',

  // Section titles
  'section.parcours': 'Mon parcours',
  'section.parcours.subtitle': 'De la passion au produit',
  'section.experience': 'Expérience professionnelle',
  'section.experience.subtitle': 'Plus de 15 ans dans l\'informatique',
  'section.skills': 'Compétences',
  'section.skills.subtitle': 'Techniques et humaines',
  'section.realisations': 'Réalisations',
  'section.realisations.subtitle': 'Projets personnels et passions',
  'section.engagements': 'Engagements',
  'section.engagements.subtitle': 'Climat, sport et transmission',
  'section.education': 'Formation',
  'section.languages': 'Langues',
  'section.certifications': 'Certifications',

  // Parcours
  'parcours.intro': 'Un chemin guidé par la curiosité, la passion du partage et l\'envie de construire.',

  'parcours.epitech.title': 'EPITECH - Apprendre à apprendre',
  'parcours.epitech.period': '2005 - 2010',
  'parcours.epitech.text': 'Passionné d\'informatique depuis l\'âge de 8 ans, le choix d\'EPITECH était une évidence après un Bac Scientifique option Sciences de l\'Ingénieur. Une école dont la philosophie repose sur un principe simple : apprendre à apprendre. L\'informatique évolue trop vite pour se reposer sur ses acquis.',

  'parcours.alternance.title': 'L\'alternance et le monde professionnel',
  'parcours.alternance.period': '2007 - 2010',
  'parcours.alternance.text': 'Dès le stage de première année en développement web, j\'ai voulu mettre un pied dans le monde du travail. En deuxième année, mi-temps entre entreprise et école. À partir de la troisième année, contrat de professionnalisation : trois jours en entreprise, deux à l\'école, les week-ends pour compenser. Au fil de ces années, j\'ai évolué du développement vers l\'administration système.',

  'parcours.debut.title': 'Premiers postes et montée en compétences',
  'parcours.debut.period': '2010 - 2012',
  'parcours.debut.text': 'De 2010 à 2012, j\'ai travaillé comme ingénieur système, gérant des plateformes e-commerce à fort trafic (Etam, Christian Louboutin, The Kooples...), avant de prendre un rôle de Technical Lead.',

  'parcours.enovance.title': 'eNovance - La pointe du Cloud',
  'parcours.enovance.period': '2012 - 2014',
  'parcours.enovance.text': 'En 2012, j\'ai rejoint eNovance, une entreprise à la pointe en matière d\'hébergement et de cloud computing, où je travaillais déjà à moitié en télétravail. Une immersion dans l\'écosystème OpenStack et les pratiques DevOps les plus avancées.',

  'parcours.redhat.title': 'Red Hat - L\'échelle mondiale',
  'parcours.redhat.period': '2014 - 2015',
  'parcours.redhat.text': 'En 2014, eNovance a été rachetée par Red Hat. L\'expérience de la grande entreprise américaine et de l\'open source à l\'échelle mondiale. Leur façon de travailler ne me convenait pas.',

  'parcours.cycloid.title': 'Cycloid - Construire un produit',
  'parcours.cycloid.period': '2015 - Aujourd\'hui',
  'parcours.cycloid.text': 'En 2015, j\'ai rejoint Cycloid comme premier salarié et associé. En tant que CTO, j\'ai construit l\'équipe technique et porté la stratégie produit. L\'entreprise a fait le choix du full remote dès le départ, en recrutant à travers l\'Europe avec l\'anglais comme langue de travail. Aujourd\'hui, je gère une équipe de 15 personnes. En 2019, une levée de fonds de plus de 3 millions d\'euros. En 2025, une deuxième levée du même montant.',

  'parcours.covid.title': 'Le Covid - Un déclic',
  'parcours.covid.period': '2020',
  'parcours.covid.text': 'Le Covid est arrivé en 2020. Déjà prêts grâce au télétravail, mais un moment de prise de conscience. C\'est à cette période que j\'ai eu un déclic climatique. Je me suis formé aux enjeux environnementaux et suis devenu animateur de la Fresque du Climat, de la Fresque du Numérique, de l\'atelier 2 Tonnes, de Nos Vies Bas Carbone.',

  'parcours.sport.title': 'Le retour au sport et la transmission',
  'parcours.sport.period': '2021 - Aujourd\'hui',
  'parcours.sport.text': 'À la sortie du Covid, retour au sport avec le taekwondo. Depuis : ceinture noire, 25 kg perdus, diplôme d\'assistant club, et investissement comme secrétaire de l\'association (~250 membres). J\'assiste le professeur et donne des cours ponctuellement. C\'est au sein de ce club que j\'ai découvert l\'amour du partage et de la transmission.',

  'parcours.apprendre.title': 'La soif d\'apprendre, toujours',
  'parcours.apprendre.period': 'Toujours',
  'parcours.apprendre.text': 'Je n\'ai jamais cessé d\'apprendre : le japonais pendant le Covid (niveau A2), l\'harmonica récemment, des papiers scientifiques sur l\'intelligence artificielle et l\'environnement. L\'informatique reste et restera une passion, désormais au service de projets qui me tiennent à cœur.',

  // Experience details
  'experience.present': 'Aujourd\'hui',
  'experience.duration.years': 'ans',
  'experience.duration.months': 'mois',

  // Projects
  'projects.visit': 'Visiter',
  'projects.tech': 'Technologies',

  // Engagements categories
  'engagements.climate': 'Climat & Environnement',
  'engagements.sport': 'Taekwondo & Sport',
  'engagements.teaching': 'Partage & Transmission',
  'engagements.opensource': 'Open Source',

  // Contact
  'contact.title': 'Contact',
  'contact.subtitle': 'N\'hésitez pas à me contacter',
  'contact.email': 'Email',
  'contact.linkedin': 'LinkedIn',
  'contact.github': 'GitHub',

  // Footer
  'footer.rights': 'Tous droits réservés.',
  'footer.built': 'Construit avec',

  // Theme
  'theme.light': 'Mode clair',
  'theme.dark': 'Mode sombre',

  // Lang
  'lang.switch': 'English',
  'lang.current': 'FR',

  // CV
  'cv.profile': 'Profil',
  'cv.profile.text': 'Plus de 15 ans d\'expérience en informatique, de l\'administration système au Cloud et DevOps. Premier salarié et associé de Cycloid, je porte la stratégie produit et dirige une équipe de 15 personnes en full remote à travers l\'Europe. Engagé pour le climat et la transmission.',
  'cv.experience': 'Expérience',
  'cv.education': 'Formation',
  'cv.skills': 'Compétences',
  'cv.languages': 'Langues',
  'cv.certifications': 'Certifications',
  'cv.engagements': 'Engagements',
  'cv.hobbies': 'Loisirs',
  'cv.contact': 'Contact',

  // Misc
  'misc.seeMore': 'En savoir plus',
  'misc.backHome': 'Retour à l\'accueil',
  'misc.downloadCV': 'Télécharger le CV',
} as const;
